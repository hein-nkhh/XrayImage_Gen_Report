...
class EnhancedCLIPVisionEncoder(nn.Module):
    ...
    def _extract_features(self, images):
        inputs = self.clip_processor(images=images, return_tensors="pt").to(Config.device)
...
class EnhancedBioBART(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        # Coverage mechanism
        self.coverage_weights = nn.Parameter(torch.randn(1, 8, 1, 64))  # For 8 attention heads
        self.coverage_loss = nn.MSELoss()

    def forward(self, vis_features, reports):
        # Project visual features
        vis_ctx = self.vis_proj(vis_features).unsqueeze(1)  # [B, 1, D]
